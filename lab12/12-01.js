...
http.createServer((req, res) => {
   ...
   if(req.method==='GET' && req.url==='/') {
       ...
       req.method==='GET' &&
       url.parse(req.url).pathname.split('/').length === 2 &&
       !isNaN(Number(url.parse(req.url).pathname.split('/')[url.parse(req.url).pathname.split('/').length - 1]) )) {
...
//
// });
// app.del('/backup/:date', (request, response) => {
//     fs.readdir(__dirname+'/backup', (err, files) => {
//         if (err) {
